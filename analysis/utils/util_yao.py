...
def count_edits(sent):
    ...
    for type_ in mapping.keys():
        if type_ == "substitution" or type_ == 'deletion' or type_ == 'reorder':
            sub_spans = get_spans_by_type(sent['original_spans'], type_)
            counts = [x[3] for x in sub_spans]
            count = len(set(counts))
            ...
            sub_spans = get_spans_by_type(sent['original_spans'], type_)
            sub_spans += get_spans_by_type(sent['simplified_spans'], type_)
            counts = [x[3] for x in sub_spans]
            count = len(set(counts))
        out[type_] = count
...
def count_composite_edits(sent, parent_id, edit_type):
    ...
    for type_ in mapping.keys():
        ...
        orig = [x for x in orig if len(x) > 4 and x[3] == parent_id and x[4] == mapping[type_]]
        simp = [x for x in simp if len(x) > 4 and x[3] == parent_id and x[4] == mapping[type_]]

        counts = list(set([x[5] for x in orig] + [x[5] for x in simp]))
        count = len(counts)
