...
def associate_spans(sent):
    ...
    for type_ in counts.keys():
        ...
        for i in annotations.keys():
            i = int(i)
            # get_span_metadata add 1 to the ID for split edits, so here we use i + 1
            if type_ == "split":
                # Get all spans corresponding to the ID
                orig_span = [x for x in orig_spans if x['id'] == i + 1 and x['type'] == type_]
                simp_span = [x for x in simp_spans if x['id'] == i + 1 and x['type'] == type_]
            else:
                # Get all spans corresponding to the ID
