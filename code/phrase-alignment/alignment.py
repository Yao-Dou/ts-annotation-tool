...
class NeuralWordAligner(nn.Module):
	def __init__(self, args):
		...
		self.bert_model = BertModel.from_pretrained('C:/Users/heine/Documents/research/phrase-alignment/code/spanbert_hf_base', output_hidden_states=True, output_attentions=True)
		# self.bert_model = BertModel.from_pretrained('SpanBERT/spanbert-large-cased', output_hidden_states=True, output_attentions=True)
...
async def inference(input_text, output_txt_file_name):
    ...
    for step, batch in enumerate(test_dataloader):
        ...
        # print(f'{input_ids_a_and_b}, {input_ids_b_and_a}, {input_mask}, {segment_ids_a_and_b}, {segment_ids_b_and_a}, {sent1_valid_ids}, {sent2_valid_ids}, {sent1_wordpiece_length}, {sent2_wordpiece_length}')
        with torch.no_grad():
...
def init_main():
	...
	yaml_cfg = parse_yaml("C:/Users/heine/Documents/research/phrase-alignment/code/"+"configs/basic.yaml")
	return yaml_cfg
...
def single_inference(sent1, sent2):
    source, target = sent1.strip(), sent2.strip()
    example = namedtuple('example', 'ID, text_a, text_b, label')
    data = [example(0, source, target, '0-0')]
    test_dataloader = create_Data_Loader(data_examples=data, set_type='test', batchsize=1, max_seq_length=128, tokenizer=tokenizer)
    for step, batch in enumerate(test_dataloader):
        batch = tuple(t.to(my_device) for t in batch)
        input_ids_a_and_b, input_ids_b_and_a, input_mask, segment_ids_a_and_b, segment_ids_b_and_a, sent1_valid_ids, sent2_valid_ids, sent1_wordpiece_length, sent2_wordpiece_length = batch
        with torch.no_grad():
            decoded_results = model(input_ids_a_and_b=input_ids_a_and_b, input_ids_b_and_a=input_ids_b_and_a,
                                    ...
                                    sent1_wordpiece_length=sent1_wordpiece_length,
                                    sent2_wordpiece_length=sent2_wordpiece_length)
        return list(decoded_results[0])
    

# format of batch: [(sent1, sent2), (sent1, sent2), ...]
def batch_inference(batch):
    example = namedtuple('example', 'ID, text_a, text_b, label')
    data = [example(0, source.strip(), target.strip(), '0-0') for source, target in batch]
    test_dataloader = create_Data_Loader(data_examples=data, set_type='test', batchsize=len(data), max_seq_length=128, tokenizer=tokenizer)
    for step, batch in enumerate(test_dataloader):
        batch = tuple(t.to(my_device) for t in batch)
        input_ids_a_and_b, input_ids_b_and_a, input_mask, segment_ids_a_and_b, segment_ids_b_and_a, sent1_valid_ids, sent2_valid_ids, sent1_wordpiece_length, sent2_wordpiece_length = batch
        with torch.no_grad():
            decoded_results = model(input_ids_a_and_b=input_ids_a_and_b, input_ids_b_and_a=input_ids_b_and_a,
                                    ...
                                    sent1_wordpiece_length=sent1_wordpiece_length,
                                    sent2_wordpiece_length=sent2_wordpiece_length)
        return [list(x) for x in decoded_results]
...
checkpoint = torch.load('C:/Users/heine/Documents/research/phrase-alignment/code/checkpoints/Checkpoint_sure_and_possible_True_dataset_mtref_batchsize_1_max_span_size_4_use_transition_layer_False_epoch_2_0.9150.pt', map_location=my_device)
...
def is_par(edit):
    return edit[0] is not None and edit[1] is not None

def is_add(edit):
    return edit[0] is None and edit[1] is not None

# problem: a change is detected as a punctuation (, . ! ?) 
# being replaced by a space and punctuation

punctuation = [".", ",", "!", "?", ")", "("]

def capitalizationChange(before, after):
    return before.lower() == after.lower()

# "Fame," -> "fame ," is not an edit
def addedSpaceAfterPunctuation(before, after):
    # if a word of length l:
    if (
        # ends in punctuation
        before[-1] in punctuation and after[-1] in punctuation and

        # is the same word (lowercased) up to l-1
        before[:-1].lower() == after[:-2].lower() and

        # has a space at the character l-1
        after[-2] == " " and

        # has the same punctuation at l
        before[-1] == after[-1]
    ):
        return True
    return False

# This is a problem. In the above example, "Fame," actually becomes "fame " and ","
# The prior gets removed and we're only left with ",", which is still invalid
# this is controversial and could remove more edits than intended
def becomesPunctuation(after):
    return after in punctuation

# "Radio." -> "Radio" is not an edit
def removedPunctuation(before, after):
    return before[-1] in punctuation and before.lower()[:-1] == after.lower()

# "Eddie Guerrero" -> "eddie guerrero" is not an edit
def lowerCased(before, after):
    return before.lower() == after.lower()

# "(with" -> "( with" is not an edit
# "(with" -> "with" is not an edit
def openParenthesisAddedBefore(before, after):
    return (before[0] == "(" and after[0] == "(" and before[1:].lower() == after[2:].lower() and after[1] == " ") or \
    (before[0] == "(" and before[1:].lower() == after.lower())

def isInvalidParChange(before, after):
    return capitalizationChange(before, after) or \
    addedSpaceAfterPunctuation(before, after) or \
    removedPunctuation(before, after) or \
    lowerCased(before, after) or \
    openParenthesisAddedBefore(before, after) or \
    becomesPunctuation(after)

def isInvalidAddChange(after):
    # "" -> "," is not an edit?
    return after in punctuation

def delete_invalid_changes(entry, mapped_changes):
    i = 0
    while i < len(mapped_changes):
        change = mapped_changes[i]
        if is_par(change) and isInvalidParChange(entry[0][change[0][0]:change[0][1]], entry[1][change[1][0]:change[1][1]]):
            del mapped_changes[i]
            print(f"Deleted: {entry[0][change[0][0]:change[0][1]]}->{entry[1][change[1][0]:change[1][1]]}")
            i -= 1
        elif is_add(change) and isInvalidAddChange(entry[1][change[1][0]:change[1][1]]):
            del mapped_changes[i]
            i -= 1
        i += 1
    return mapped_changes

def combine_adjacent_edits(mapped_changes):
    # Combine adjacent additions or deletions
    # Could be written more efficiently
    # Combines adjacent deletions 
    i = 0
    while i < len(mapped_changes):
        change = mapped_changes[i]
        if change[1] is None:
            for j in range(len(mapped_changes)):
                next_change = mapped_changes[j]
                if next_change[1] is None and change[0][1] + 1 == next_change[0][0]:
                    mapped_changes[i][0][1] = next_change[0][1]
                    del mapped_changes[j]
                    i -= 1
                    break
        i += 1

    # Combines adjacent additions 
    i = 0
    while i < len(mapped_changes):
        change = mapped_changes[i]
        if change[0] is None:
            for j in range(len(mapped_changes)):
                next_change = mapped_changes[j]
                if next_change[0] is None and change[1][1] + 1 == next_change[1][0]:
                    mapped_changes[i][1][1] = next_change[1][1]
                    del mapped_changes[j]
                    i -= 1
                    break
        i += 1

    # Combine adjacent paraphrases
    i = 0
    while i < len(mapped_changes) - 1:
        if is_par(mapped_changes[i]):
            new_change = copy.deepcopy(mapped_changes[i])
            j = i + 1
            # I should also check to make sure the next change is within or next to the current change
            # important note: many to one relationships could exist if their inputs are not adjacent
            while j < len(mapped_changes) and is_par(mapped_changes[j]):
                next_change = copy.deepcopy(mapped_changes[j])
                if next_change[1][0] >= new_change[1][0] and next_change[1][1] <= new_change[1][1]:
                    # output is within new_change's bounds
                    new_change[0][1] = next_change[0][1]
                    del mapped_changes[j]
                elif next_change[1][1] + 1 == new_change[1][0]:
                    # output is directly before new_change
                    new_change[0][1] = next_change[0][1]
                    new_change[1][0] = next_change[1][0]
                    del mapped_changes[j]
                elif new_change[1][1] + 1 == next_change[1][0]:
                    # ouput is directly after new_change
                    new_change[0][1] = next_change[0][1]
                    new_change[1][1] = next_change[1][1]
                    del mapped_changes[j]
                else:
                    break
            mapped_changes[i] = new_change
        i += 1
    return mapped_changes

def naive_indices(tokens):
    indices, i = [], 0
    for sent in tokens:
        # print(f'{curr}-{curr+len(i)}:\t {sent1[curr:curr+len(i)]}')
        indices.append([i, i+len(sent)])
        i += len(sent) + 1
    return indices

def batch_alignment(batch):
    json_output = []
    id_ = 0
    outs = batch_inference(batch)
    for out in outs:
        tups = [(x[0], x[1]) for x in sorted([[int(i) for i in x.split('-')] for x in out], key = lambda x: x[0])]
        s1_tokens, s2_tokens = batch[id_][0].split(" "), batch[id_][1].split(" ")
        s1_tokens_ind, s2_tokens_ind = naive_indices(s1_tokens), naive_indices(s2_tokens)
        mapped_s1_tokens, mapped_s2_tokens = set([x[0] for x in tups]), set([x[1] for x in tups])
        
        # Remove changes which didn't effect the sentence
        changes = [(a, b) for a, b in tups if s1_tokens[a] != s2_tokens[b]] 

        # Add additions and deletions mappings
        changes += [(i, None) for i in range(len(s1_tokens)) if i not in mapped_s1_tokens]
        changes += [(None, i) for i in range(len(s2_tokens)) if i not in mapped_s2_tokens]
        
        # Sort. Rule: by change[0] unless it's an addition, then just use the change[1] entry
        changes = sorted([x for x in changes], key=lambda x: (x[0] if x[0] is not None else x[1], x[1] if x[1] is not None else x[0]))

        # Map changes to corresponding indices
        mapped_changes = [[s1_tokens_ind[x[0]] if x[0] is not None else None, s2_tokens_ind[x[1]] if x[1] is not None else None] for x in changes]
        
        # Delete edge cases of clearly invalid paraphrase alignments
        mapped_changes = delete_invalid_changes(batch[id_], mapped_changes)
        
        # New: Combine adjacent alignments
        mapped_changes = combine_adjacent_edits(mapped_changes)
        
        # Add aligned sentence to json
        json_output.append({
            "ID": id_,
            "Original": batch[id_][0],
            "Simplification": batch[id_][1],
            "Alignment": mapped_changes
        })
        
        id_ += 1
    return json_output

# Load and process a file of sentences
import sys
if len(sys.argv) != 3:
    raise RuntimeError("Must specify starting and ending indicies of data to process.")

first_sent_idx, last_sent_idx = int(sys.argv[1]), int(sys.argv[2])

with open("data/input_yao.json") as f:
    input_data = json.load(f)

# Limit the number of sents processed
input_data = input_data[first_sent_idx:last_sent_idx]
    
raw_sents = [(x['original'], x['simplified']) for x in input_data]

import gc
n = 1
aligned = []
for i in range(int(len(raw_sents) / n)):
	aligned += batch_alignment(raw_sents[i*n:(i+1)*n])
	gc.collect()

# Rip apart the output aligned array and insert sentence metadata
# for i in range(len(aligned)):
# 	aligned[i]["ID"] = i
# 	aligned[i]["Batch"] = input_data[i]["Batch"]
# 	aligned[i]["ID_from_batch"] = input_data[i]["ID"]
# 	aligned[i]["Rating"] = input_data[i]["Rating"]
# 	aligned[i]["System"] = input_data[i]["System"]

for i in range(len(aligned)):
	newDict = {}

	newDict['id'] = first_sent_idx + i
	newDict["original"] = aligned[i]["Original"]
	newDict["simplified"] = aligned[i]["Simplification"]
	newDict["system"] = input_data[i]["system"]
	newDict["spans"] = aligned[i]["Alignment"]

	aligned[i] = newDict

with open(f'data/input_yao_aligned_{first_sent_idx}_{last_sent_idx}.json', 'w', encoding='utf-8') as f:
